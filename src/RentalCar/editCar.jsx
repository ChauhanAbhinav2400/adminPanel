...
function EditCar() {
  ...
  const getNewFormData = useSelector(
    (state) => state.CarRentalEditReducer.editFormData
  );
  const [oemNameSelected, setOemNameSelected] = useState();
  const [oemCarModelSelected, setCarModelSelected] = useState();
  const [oemCitySelected, setCitySelected] = useState();
  ...
  //     formData.cityNm !== "" &&
  //     getNewFormData &&
  //     formData.modelNm !== undefined
  //   ) {
  //     addCar(formData)
  //       .then((response) => {
  //         console.log(response);
  //         console.log(response.data.id);
  //         setOemNameSelected(response.data.oemNm);
  //         setCarModelSelected(response.data.modelNm);
  //         setCitySelected(response.data.cityNm);
                ...
                <div className=" full flex flex-col  w-full">
                  ...
                  >
                    ...
                    {OemName.map((item, index) => {
                      if (formData.oemNm === item.oemName) {
                        return (
                  ...
                  <div className=" flex flex-col w-full mt-6">
                    ...
                    >
                      ...
                      {CityList.map((item, index) => {
                        if (formData.cityNm === item.cityNm) {
                          return (
                            <option key={index} selected>
                              {item.cityNm}
                            </option>
                          ...
                          return <option key={index}>{item.cityNm}</option>;
                        }
                      })}
                    </select>
                  </div>
                ...
                <div className=" w-3/6 flex flex-col max-md:w-full">
                  ...
                  >
                    ...
                    {transmission.map((item, index) => {
                      if (formData.transmission === item.transmission) {
                        return (
                          <option key={index} selected>
                            {item.transmission}
                          </option>
                ...
                <div className=" w-3/6 flex flex-col max-md:w-full">
                  ...
                  >
                    ...
                    {fuelName.map((item, index) => {
                      if (formData.fuelName === item.fuelName) {
                        return (
                          <option key={index} selected>
                            {item.fuelName}
                          </option>
