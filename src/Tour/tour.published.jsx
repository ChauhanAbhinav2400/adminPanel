...
import { getTourById } from "../redux/actions/action.edit";
...
const PublishedTour = () => {
  ...
  const [publishedTour, setPublishedTour] = useState([]);
  const [firstvalue, setFirstValue] = useState(0);
console.log(publishedTour)
  useEffect(() => {
    getPublishedTour(firstvalue)
      .then((response) => {
        setPublishedTour(response?.data?.content);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    console.log("hit");
    if (filteredDate) {
      const filteredTours = publishedTour.filter(
        (article) => article.createdDt?.slice(0, 11) === filteredDate
      );
      setPublishedTour(filteredTours);
    }

    if (inputValue) {
      const filterByNameArray = publishedTour.filter((article) => {
        return article.title.includes(inputValue);
      });

      setPublishedTour(filterByNameArray);
    }
  ...
  const handleEdit = (id, title) => {
    ...
    navigate(`/admin/tour-package/edit/${encodeURIComponent(id)}`);
  };

  const handleFeatured = (id) => {
    CreateFeaturedTour(id)
      .then((response) => {
        if (response.data === true) {
          toast.success("Featured SuccessFully");
          navigate("/admin/tour-package/featured");
        } else {
          toast.error("Featured Limit Full");
        }
      })
      .catch((error) => {
    ...
    <div className="bg-gray/10 relative gap-5 max-h-full overflow-y-auto  flex flex-col w-full p-4 ">
      <div className="w-full h-40 py-6 px-4 rounded-lg flex flex-col gap-5  bg-white ">
        <div className="w-full flex justify-between">
          <h1 className="text-xl font-medium rubik">Draft Articles</h1>
      ...
      <table className="w-full table-auto">
        <thead className="">
          <tr className=" w-full h-20 border-b-8 border-gray/10 text-md font-medium  bg-white rounded-lg pb-2  ">
            ...
            <th className="px-4 py-2 text-left">Article Categories</th>
            <th className="px-4 py-2 text-left">Date Of Creation</th>
            <th className="px-4 py-2 text-left">Action</th>
          </tr>
        </thead>

        <tbody>
          {publishedTour?.map((article, index) => (
            <tr key={index} className={"bg-white border-b-4  border-gray/10"}>
              ...
              <td className="px-4 py-2 border-r-2 border-gray/10 text-right">
                {article.wordCount}
              </td>
              <td className="px-4 py-2 border-r-2 border-gray/10">
              ...
              <td className="px-4 py-2">
                <div className="flex space-x-2">
                  <button
                    onClick={() => handleFeatured(article.id)}
                    className="bg-primary hover:bg-blue-600 text-white px-2 py-1 rounded-md"
                  >
                    <IoIosStar size={22} color="white" />
                  </button>
                  <button
