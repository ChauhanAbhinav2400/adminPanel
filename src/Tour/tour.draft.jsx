...
import { getDraftTourById } from "../redux/actions/action.edit";
import { toast } from "react-toastify";
import { getDeleteTour, getDraftTour } from "../ApiListener";
...
const DraftTour = () => {
  ...
  useEffect(() => {
    getDraftTour(firstvalue)
      .then((response) => {
        setDraftArticle(response?.data?.content);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    console.log("hit");
    if (filteredDate) {
      const filteredArticles = draftArticle.filter(
        (article) => article.createdDt?.slice(0, 11) === filteredDate
      );
      setDraftArticle(filteredArticles);
    }

    if (inputValue) {
      const filterByNameArray = draftArticle.filter((article) => {
        return article.title.includes(inputValue);
      });

      setDraftArticle(filterByNameArray);
    }
  ...
  return (
    <div className="bg-gray/10 relative gap-5 max-h-full overflow-y-auto  flex flex-col w-full p-4 ">
      ...
      <table className="w-full table-auto">
        ...
        <tbody>
          {draftArticle?.map((article, index) => (
